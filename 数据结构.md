...
1.调用函数时，系统会为调用者构造一个由参数表和返回地址组成的活动记录，并将记录压入系统提供的栈中，若被调用函数有局部变量，也要压入栈中。
...
1. 在二叉树中有两个结点m和n，若m是n的祖先，则使用**后序遍历**可以找到m到n的路径；
...
假设以Nh表示深度为h的平衡树中含有的最少结点数，则有N0=0，N1=1，N2=2，并且
$$
n_h=n_{h-1}+n_{h-2}+1
$$
...
在有n的叶子结点的哈夫曼树中，非叶子结点的总数是n-1；

一棵度为m的哈夫曼树应只有度为0和m的结点；



### ==5.5.4 并查集==

并查集的存储结构是用**双亲表示法**存储的树；
...
在长度为n的并查集中进行查找操作的时间复杂度为O（n）；

**并查集优化：**

1. 每次合并都让小树合并到大树上，这样可以保证树的高度不会增高，高度不超过$\lfloor log_2n \rfloor$+1;
2. 用根节点的绝对值表示一棵树（或集合）的结点个数；
3. find优化：压缩路径；

![image-20220808193743460](C:\Users\24504\AppData\Roaming\Typora\typora-user-images\image-20220808193743460.png)
...
对于n个顶点的无向图G，若G是连通图，则最少有n-1条边，若G是非连通图，则最多可能有C<sup>2</sup><sub>n-1</sub>条边；
...
有向邻接矩阵中，0和∞表示的都不是有向边；

入度是由邻接矩阵的列中元素计算出来的，出度是由邻接矩阵的行中元素计算出来的；
...
```c
#define MaxVertecNum 100                //顶点书目最大值
typedef struct
{
...
设图G的邻接矩阵为A（矩阵元素为0/1），则A<sup>n</sup>的元素A^n^[i][j]等于由顶点i到顶点j的长度为n的路径的数目；

![image-20220227192412021](C:\Users\24504\AppData\Roaming\Typora\typora-user-images\image-20220227192412021.png)
...
|                  | 邻接表                                     | 邻接矩阵             | 十字链表       | 邻接多重表      |
| ---------------- | ------------------------------------------ | -------------------- | -------------- | --------------- |
| 空间复杂度       | 无向图O(\|V\|+2\|E\|);有向图O(\|V\|+\|E\|) | O(\|V\|<sup>2</sup>) | O(\|V\|+\|E\|) | O(\|V\|+\|E\|） |
| 适合用于         | 存储稀疏图                                 | 存储稠密图           | 很方便         | 很方便          |
| 表示方法         | 不唯一                                     | 唯一                 | 很方便         | 很方便          |
| 计算度/出度/入度 | 计算有向图的度、入度不方便，其余很方便     | 必须遍历对应行或列   | 只能存有向图   | 只能存无向图    |
| 找相邻的边       | 找有向图的入边不方便，其余很方便           | 必须遍历对应行或列   | 不唯一         | 不唯一          |
...
**NextNeighbor(G,x,y):**假设图G中顶点y是顶点x的一个邻接点，返回除y之外顶点x的下一个邻接点的顶点号，若y是x的UI后一个邻接点，则返回-1.
...
时间复杂度：邻接矩阵：O(|V|<sup>2</sup>) ;

​						邻接表：O(|V|+|E|)



对于同一个图，基于邻接矩阵的遍历所得的DFS序列和BFS序列是惟一的，基于邻接表的遍历所得到的的DFS序列和BFS序列是不唯一的；

利用深度优先遍历可以判断图G中是否存在==回路==；对于无向图来说，若深度优先遍历过程中遇到了回边，则必定存在环；对于有向图来说，这条回边可能是指向深度优先森林中另一棵生成树上的顶点的弧；



### 6.3.3 图的遍历与图的连通性

对无向图进行BFS/DFS遍历，调用BFS/DFS函数的次数=联通分量数；对于连通图，只需调用1次BFS/DFS。

对于有向图进行BFS/DFS遍历，若起始顶点到其他顶点都有路径，则只需调用1次BFS/DFS函数，对于强连通图，从任意结点出发都只需调用1次BFS/DFS

## 6.4 图的应用

### 6.4.1 最小生成树

**Prim算法：**时间复杂度：O(|V|^2^),适合用于边稠密图；

**Kruskal算法:**时间复杂度：O(|E|log~2~|E|),适合用于边稀疏图；（与并查集结合学习）
...
![image-20220228175935421](C:\Users\24504\AppData\Roaming\Typora\typora-user-images\image-20220228175935421.png)![image-20220228175957486](C:\Users\24504\AppData\Roaming\Typora\typora-user-images\image-20220228175957486.png)

Dijkstra算法不适用于有负权值的带权图；

Dijkstra算法适合求解有回路的带权图的最短路径，也可以求任意两个顶点的最短路径；

**Floyd算法：**

在用Floyd算法求两个顶点的最短路径时，当最短路径发生更改时，path<sub>k-1</sub>就不是path<sub>k</sub>的子集；



![image-20220302143209678](C:\Users\24504\AppData\Roaming\Typora\typora-user-images\image-20220302143209678.png)![image-20220302143226663](C:\Users\24504\AppData\Roaming\Typora\typora-user-images\image-20220302143226663.png)![image-20220302143245877](C:\Users\24504\AppData\Roaming\Typora\typora-user-images\image-20220302143245877.png)
...
| 通常用于       | 求无权图的单源最短路径               | 求带权图的单源       | 求带权图中各顶点间的最短路径 |
...
![image-20220810185106778](C:\Users\24504\AppData\Roaming\Typora\typora-user-images\image-20220810185106778.png)
...
bool TopologicalSort(GRAph G)
{
	...
	while(!IsEmpty(S))       //栈不空，则存在入度为0的顶点
    {
        ...
        {
            //将所有i指向的顶点的入度建1，并且将入度减为0的顶点压入栈S
            v=p->adjvex;
            if(!(--indegree[v]))
                Push(S,v);      //入度为0，则入栈
        }
...
![image-20220811164310512](C:\Users\24504\AppData\Roaming\Typora\typora-user-images\image-20220811164310512.png)

### 6.4.5 关键路径

**AOE网：**在AOE网中仅有一个入度为0的顶点，称为开始顶点（源点），他表示整个工程的开始；也仅有一个出度为0的顶点，称为结束顶点（汇点），他表示整个工程的结束。

## 6.5 补充知识点

1. ==深度优先遍历、拓扑排序、求关键路径==都可以判断一个有向图是否有环。
...
有序线性表的顺序查找和折半查找的思想是不一样的，且有序线性表的顺序查找中的线性表可以是链式存储结构的；
...
折半查找仅适用于有序的顺序表，不适用于链式表；

折半查找的判定树一定是平衡二叉树；

概要: 具有N个（N>0）结点的完全二叉树的高度为 $\lceil$log<sub>2</sub>(N+1)$\rceil$ 或 $\lceil$log<sub>2</sub>N$\rceil$ +1。

折半查找二叉树：右子树结点数-左子树结点数=0或1；

折半查找等概率下，查找成功的平均查找长度是ASL=**log~2~(n+1)-1**;
...
长度为n的查找表被均匀地分成b块，每块s个元素；

由于分块查找实际是进行两次查找，所以整个算法的平均查找长度是两次查找的平均查找长度之和。 即ASL=L<sub>I</sub>+L<sub>S</sub> =(b+1)/2+(s+1)/2

顺序查找索引表：当s=$\sqrt{n}$时，ASL<sub>最小</sub>=$\sqrt{n+1}$；
...
含有n个结点平衡二叉树的最大深度为O(log<sub>2</sub>n)，因此，平衡二叉树的平均查找长度为O(log<sub>2</sub>n)

平衡二叉树的递推公式：n<sub>0</sub>=0,n<sub>1</sub>=1,n<sub>2</sub>=2,**n~h~=1+n~h-1~+n~h-2~**;(h为平衡二叉树高度，n<sub>h</sub>为构造此平衡二叉树所需的最小结点数)
...
4. 对每个结点，从该节点到任一叶结点的简单路径上，所含黑结点的数量相同；
...
![image-20220812174917135](C:\Users\24504\AppData\Roaming\Typora\typora-user-images\image-20220812174917135.png)
...
![image-20220813150734902](C:\Users\24504\AppData\Roaming\Typora\typora-user-images\image-20220813150734902.png)



B树是一种平衡的多路查找树，2-3树和2-3-4树都是B树的特例，我们把树中结点最大的孩子数目称为B树的阶。通常记为m。 一棵m阶B树或为空树，或为满足如下特性的m叉树：

- 1）树中每个结点至多有m棵子树。（即至多含有m-1个关键字) ("两棵子树指针夹着一个关键字")
- 2）若根结点不是终端结点，则至少有两棵子树。(至少一个关键字)
- 3）除根结点外的所有非叶结点至少有 ⌈m/2⌉棵子树。（即至少含有⌈m/2⌉-1个关键字）
- 4）所有的叶子结点出现在同一层次上，不带信息。(就像是折半查找判断树中查找失败的结点)
...
含n个关键字的m阶B树，高度为==log~m~(n+1)<=h<=log~⌈m/2⌉~（(n+1)/2）+1==;

h层m阶B树至少包含的关键字总数是：$n>=2*\lceil m/2\rceil^{h-1}-1$

**含有n个关键字的B树中含有的叶结点数为n+1；**
...
![image-20220813150643198](C:\Users\24504\AppData\Roaming\Typora\typora-user-images\image-20220813150643198.png)
...
1. 每个分支点最多有m课子树（孩子结点）；
2. 非叶根节点至少有两课子树，其他每个分支结点至少有⌈m/2⌉棵子树；
3. 结点的子树个数与关键字个数相等；
4. 所有叶结点包含全部关键字及指向相应记录的指针，叶结点中将关键字按大小顺序排列，并且相邻叶结点按大小顺序相互链接起来；
5. 所有分支结点中仅包含他的各个子节点中关键字额最大值及指向其子节点的指针；

m阶的B+树与m阶的B树的主要差异在于：

- 1）在B+树中，具有n个关键字的结点只含有n棵子树，即每个关键字对应一棵子树；

  ​      在B树中，具有n个关键字的结点含有(n+1)棵子树。

- 2）在B+树中，每个结点（非根内部结点）关键字个数n的范围是 ⌈m/2⌉≤n≤m（根结点1≤n≤m）；

  ​      在B树中，每个结点（非根内部结点）关键字个数n的范围是⌈m/2⌉ -1≤n≤m-1（根结点：1≤n≤m-1）。

- 3）在B+树中，叶结点包含信息，所有非叶结点仅起到索引作用，非叶结点中的每个索引项只含有对应子树的        最大关键字和指向该子树的指针，不含有该关键字对应记录的存储地址。 
...
​	装填因子a=表中记录数/散列表长度  （装填因子会直接影响散列表的查找效率）
...
**1.直接定址法：**直接取关键字的某个线性函数值为散列地址，散列函数为H(key)=a×key+b。式中，a和b是常数。这种方法计算最简单，并且不会产生冲突
...
1）线性探测法：冲突发生时，顺序查看表中下一个单元（当探测到表尾地址m-1时，下一个探测地址是表首地址0），直到找出一个空闲单元（当表未填满时一定能找到一个空闲单元）或查遍全表。**线性探测法容易引起聚集现象；**

2）平方探测法：设发生冲突的地址为d,平方探测法得到的新的地址序列为d+1^2^，d-1^2^，d+2^2^，d-2^2^...... 平方探测法是一种较好的处理冲突的方法，可以避免出现“堆积”问题，它的缺点是不能探测到散列表上的所有单元，但至少能探测到一半单元。

3）双散列法：又称为再散列法。需要使用两个散列函数，当通过第一个散列函数H(Key)得到的地址发生冲突时，则利用第二个散列函数Hash2(Key)计算该关键字的地址增量。**再散列法处理冲突时不易产生聚集；**

4）伪随机序列法：当发生地址冲突时，地址增量为伪随机数序列，称为伪随机序列法。

2.拉链法：对于不同的关键字可能会通过散列函数映射到同一地址，为了避免非同义词发生冲突，可以把所有的同义词存储在一个线性链表中，这个线性链表由其散列地址唯一标识。拉链法适用于经常进行插入和删除的情况。**拉链法不会引起聚集现象；**
...
散列表的平均查找长度依赖于散列表的装填因子α，而不是直接依赖于n或m；

α越大，表示装填的记录越“满”，发生冲突的可能性就越大，反之发生冲突的可能性越小；

采用开放定址法解决冲突的散列查找中，发生聚集的原因主要是：**解决冲突的方法选择不当**。

在开放定址法中，散列到同一个地址而引起的“堆积”问题是由于**同义词或非同义词之间发生冲突，主要是非同义词之间的冲突；**
...
对任意n个关键字排序的比较次数至少是$\lceil{log_2(n!)} {\rceil}$;
...
更适用于基本有序的排序表和数据量不大的排序表，仅适用于线性表为顺序存储的情况，不适用于链表；
...
算法思想：从后往前（或从前往后）==两两==比较相邻元素的值，若为逆序（即A[i-1]>[i])，则交换它们，直到序列比较完。称这样的过程为“一趟”冒泡排序。

![image-20220310203817069](C:\Users\24504\AppData\Roaming\Typora\typora-user-images\image-20220310203817069.png)
...
算法思想：在待排序表L[1..n]中任取-个元素pivot作为枢轴(或基准，通常取首元素)，通过一趟排序将待排序表划分为独立的两部分L[1...k-1]和L[k+1...n]，使得L[1...k-1]中的所有元素小于pivot, L[k+1...n]中的所有元素大于等于
pivot,则pivot放在 了其最终位置L(k)上，这个过程称为一次“划分”。然后分别递归地对两个子表重复上述过程，直至每部分内只有一个元素或空为止，即所有元素放在了其最终位置上。
...
**注意：**在快速排序算法中，并不产生有序子序列，但每趟排序后会将枢轴元素放到其最终位置上。
...
算法效率分析
空间复杂度 = O(1)
无论有序、逆序、乱序，都需要==n-1==的处理，总共需要对比关键字 (n-1)+(n-2)+…+1 =n(n-2)/2 次，元素交换次数 < n-1; 
...
时间复杂度：建堆:O(n)             排序：O(nlog<sub>2</sub>n)             总时间复杂度：O(nlog<sub>2</sub>n) 
...
1. 新元素放到标尾
2. 根据大/小根堆的要求，新元素不断上移，直到无法上移为止；

堆的删除：

1. 被删除元素用表尾元素代替
2. 根据大/小根堆的要求，新元素不断下坠，直到无法下坠为止；
...
对于N个元素进行k路归并排序，排序的趟数m满足 k^m^ = N, m = ⌈log<sub>k</sub>N⌉
...
![](D:\考研\专业课\数据结构\数据结构\排序比较.png)

1. 算法复杂度与初始状态无关的是：选择排序，堆排序，归并排序，基数排序；
2. 元素总比较次数与初始状态无关的是：选择排序，基数排序；
3. 元素总移动次数与初始状态无关的是：归并排序，基数排序；
4. 排序趟数与初始状态**有关**的是：快排，冒泡排序；
5. 不宜使用链式存储的是：希尔排序，堆排序；
...
![image-20220310142913076](C:\Users\24504\AppData\Roaming\Typora\typora-user-images\image-20220310142913076.png)

**重要结论：**采用多路归并可以减少归并趟数，从而减少磁盘I/O（读写）次数；

对r个初始归并段，做k路归并，则归并树可用k叉树表示，若树高为h，则归并趟数=h-1=⌈log<sub>k</sub>r⌉;
...
![image-20220311194913415](C:\Users\24504\AppData\Roaming\Typora\typora-user-images\image-20220311194913415.png)

对于k路归并，第一次构造败者树需要对比关键字k-1次；

有了败者树，选出最小元素，只需要对比关键字⌈log<sub>2</sub>k⌉次；
...
归并树的带权路径长度WPL=读磁盘的次数=写磁盘次数

重要结论：归并过程中的磁盘I/O次数=归并树的WPL*2

![image-20220310151746026](C:\Users\24504\AppData\Roaming\Typora\typora-user-images\image-20220310151746026.png)

注意：

对于k叉归并，若初始归并段的数量无法构成严格的k叉归并树，则需要补充几个长度为0的“虚段”，再进行k叉哈夫曼树的构造。

k叉树的最佳归并树一定是一棵严格的k叉树，即树中值包含度为k、度为0的结点。

设度为k的结点有n<sub>k</sub>个，度为0的结点有n<sub>0</sub>个，归并树总结点数=n，则：

初始归并段数量+虚段数量=n<sub>0</sub>

n=n<sub>0</sub>+n<sub>k</sub>
